...
const Alert = ({ title, children }: AlertTypes) => {
    ...
    <>
      ...
      {show && (
        <div id="alert-additional-content-1" className="fixed inset-0 flex items-center justify-center z-50" onClick={() => setShow(false)}>
          ...
          <div className="relative p-8 bg-white w-screen rounded-lg shadow-lg max-w-screen-lg">
            ...
            <hr className='h-1 w-screen absolute left-0' />
            <div className="my-4 text-sm text-left font-medium space-y-3">
              {children}
            </div>
          </div>
        </div>
