...
export const CapacitiesTable = ({ capacities, setCapacities, sortConfig, requestSort, show, setShow, modalStatus, setModalStatus }: CapacitiesTableTypes) => {
    ...
    useEffect(() => {
        const date = currentCapacity?.date ? new Date(currentCapacity.date) : null;
        setCalendar({
            startDate: date,
            endDate: date
        });
    ...
    const deleteCapacityException = () => {
        fetchAuthentication.post(`/api/capacity/delete/${currentCapacity ? currentCapacity.id : ''}`).then(res => {
    ...
    const updateCapacityException = () => {
        fetchAuthentication.post('/api/capacity/update').then(res => console.log(res.data))
    }
        ...
        <div>
            ...
            {modalStatus === 'update' && (
                <Modal show={show} setShow={setShow} title='Kivétel frissítése'>
                    <form onSubmit={updateCapacityException} className="max-w-sm mx-auto pb-10">
                    ...
                    <div className='py-5'>
                        <div className="p-4 md:p-5 text-center">
                            ...
                            <div className="flex items-center justify-center">
                                <button onClick={deleteCapacityException} data-modal-hide="popup-modal" type="button" className="btn-red  flex items-center justify-center">
                                    {buttonLoading ? <MoonLoader size={20} color="rgba(255, 255, 255, 1)" /> : 'Megerősít'}
                                </button>
                                <button data-modal-hide="popup-modal" type="button" className="btn-light">Kilépés</button>
                            </div>
                        </div>
                    </div>
                </Modal>
